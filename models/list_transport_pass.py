        ...
        for name, info in data.items():
            if info[1] == 'None':
                ...
                if (staff.number_of_pass, staff.registration_number) in \
                        list(session.query(ListTransportPass.number_of_pass,
                                           ListTransportPass.registration_number)):
                    pass
                else:
                    session.add(staff)
    session.commit()
